...
from model import Dual_UFormer_Image_Deraining_Network as DUFN
...
class Session:
    def __init__(self):
        ...
        if torch.cuda.is_available():
            self.net = DUFN().cuda()
        if len(device_ids) > 1:
            self.net = nn.DataParallel(DUFN()).cuda()
        self.opt_net = Adam(self.net.parameters(), lr=settings.lr)
        self.sche_net = MultiStepLR(self.opt_net, milestones=[501*settings.one_epoch_step], gamma=0.1)
        self.ssim = SSIM().cuda()
        self.dataloaders = {}
